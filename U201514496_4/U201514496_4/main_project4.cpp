...
int main(int agrc,char* agrv[]) {
	...
	while (num_cnt < agrc) {
		if (!strcmp("-I", agrv[num_cnt])) {
			...
			++num_cnt;
			get_num = atoi(agrv[num_cnt]);
			if (get_num >= pqueue->m_iMax) {
				cout << "Errors!" << endl;
				fprintf(fp, "%c", 'E');
				fclose(fp);
				return 0;
			}
			get_num = (get_num + 1) % pqueue->m_iMax;		//	将下标转换，方便于接下来的判断
			if (pqueue->m_iHead <= pqueue->m_iTail) {
				if ((get_num <= pqueue->m_iTail) && (get_num > pqueue->m_iHead)) {		//	下标在范围内
				...
				if (((get_num >= 0) && (get_num <= pqueue->m_iTail)) || ((get_num > pqueue->m_iHead) && (get_num <= (pqueue->m_iMax - 1)))) {	//下标在范围内
					fprintf(fp, "%d  ", pqueue->operator[](get_num));
				}
				else {
					cout << "Errors!" << endl;
